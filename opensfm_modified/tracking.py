...
def load_features(feature_path, images,opensfm_config):
    logging.info('reading features')
    features = {}
    colors = {}
    for im in images:
        p, f, c = opensfm_interface.load_features(feature_path,im,opensfm_config)
        features[im] = p[:, :3]
        colors[im] = c
    return features, colors


def load_matches(file_path, images):
    matches = {}
    for im1 in images:
        try:
            #print(im1)
