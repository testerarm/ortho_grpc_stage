...
def compute_image_pairs(track_dict, cameras, data, file_path):
    """All matched image pairs sorted by reconstructability."""
    args = _pair_reconstructability_arguments(track_dict, cameras, data, file_path)
...
def _pair_reconstructability_arguments(track_dict, cameras, data, file_path):
    threshold = 4 * data['five_point_algo_threshold']
    args = []
    for (im1, im2), (tracks, p1, p2) in iteritems(track_dict):
        camera1 = cameras[opensfm_interface.load_exif(file_path,im1)['camera']]
        camera2 = cameras[opensfm_interface.load_exif(file_path,im2)['camera']]
...
def get_image_metadata(file_path, image,config):
    """Get image metadata as a ShotMetadata object."""
    metadata = types.ShotMetadata()
    exif = opensfm_interface.load_exif(file_path,image)
...
def bootstrap_reconstruction(file_path,opensfm_config, graph, camera_priors, im1, im2, p1, p2):
    ...
    camera_id1 = opensfm_interface.load_exif(file_path,im1)['camera']
    camera_id2 = opensfm_interface.load_exif(file_path,im2)['camera']
    camera1 = camera_priors[camera_id1]
    camera2 = camera_priors[camera_id2]

    threshold = opensfm_config['five_point_algo_threshold']
    min_inliers = opensfm_config['five_point_algo_min_inliers']
    iterations = opensfm_config['five_point_refine_rec_iterations']
    ...
    reconstruction.reference = opensfm_interface.load_reference(file_path)
    ...
    shot1.metadata = get_image_metadata(file_path, im1,opensfm_config)
    ...
    shot2.metadata = get_image_metadata(file_path, im2, opensfm_config)
    reconstruction.add_shot(shot2)

    graph_inliers = nx.Graph()
    triangulate_shot_features(graph, graph_inliers, reconstruction, im1, opensfm_config)
    ...
    bundle_single_view(graph_inliers, reconstruction, im2, camera_priors,
                       opensfm_config)
    retriangulate(graph, graph_inliers, reconstruction, opensfm_config)
...
class ShouldBundle:
    ...
    def __init__(self, data, reconstruction):
        self.interval = data['bundle_interval']
        self.new_points_ratio = data['bundle_new_points_ratio']
        self.reconstruction = reconstruction
        self.done()
...
class ShouldRetriangulate:
    ...
    def __init__(self, opensfm_config, reconstruction):
        self.active = opensfm_config['retriangulation']
        self.ratio = opensfm_config['retriangulation_ratio']
        self.reconstruction = reconstruction
        self.done()
...
def grow_reconstruction(file_path,opensfm_config, graph, graph_inliers, reconstruction, images, camera_priors, gcp):
    """Incrementally add shots to an initial reconstruction."""
    config = opensfm_config
    report = {'steps': []}
    ...
    should_bundle = ShouldBundle(config, reconstruction)
    should_retriangulate = ShouldRetriangulate(config, reconstruction)
    while True:
        if config['save_partial_reconstructions']:
            paint_reconstruction(data, graph, reconstruction)
            opensfm_interface.save_reconstruction(file_path,
        ...
        logger.info("-------------------------------------------------------")
        threshold = opensfm_config['resection_threshold']
        min_inliers = opensfm_config['resection_min_inliers']
        for image, num_tracks in candidates:

            camera = reconstruction.cameras[opensfm_interface.load_exif(file_path,image)['camera']]
            metadata = get_image_metadata(file_path, image, opensfm_config)
    ...
    paint_reconstruction(config, graph, reconstruction)
...
def incremental_reconstruction(file_path, graph, opensfm_config):
    ...
    if not opensfm_interface.reference_lla_exists(file_path):
        opensfm_interface.invent_reference_lla(file_path,images)

    remaining_images = set(images)
    camera_priors = opensfm_interface.load_camera_models(file_path)
    gcp = opensfm_interface.load_ground_control_points(file_path, images)

    common_tracks = tracking.all_common_tracks(graph, tracks)
    reconstructions = []
    pairs = compute_image_pairs(common_tracks, camera_priors, opensfm_config, file_path)
        ...
        if im1 in remaining_images and im2 in remaining_images:
            ...
            if reconstruction:
                ...
                reconstruction, rec_report['grow'] = grow_reconstruction(file_path,
                    opensfm_config, graph, graph_inliers, reconstruction, remaining_images, camera_priors, gcp)
