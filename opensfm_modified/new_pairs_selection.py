...
def match_candidates_with_bow(feature_path, data, images_ref, images_cand,
...
def create_parallel_matching_args(data, preempted_cand, histograms):
    ...
    args = list(match_histogram_arguments(preempted_cand, histograms))
     ...
     # parallel VLAD neighbors computation
    per_process = 512
    processes = context.processes_that_fit_in_memory(opensfm_config['processes'], per_process)
...
def match_candidates_from_metadata(file_path ,images_ref, images_cand, exifs, opensfm_config):
...
def vlad_histograms(images, opensfm_config):
    ...
    words = vlad.instance.load_words(opensfm_config)
    vlads = {}
    for im in images:
